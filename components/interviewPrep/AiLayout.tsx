...
export default function AiLayout({ expand, startListen, stopListen, interimText, listen, setInterimText, setResponse, setListen, speaking }: listener) {
    return <div className="w-full flex flex-col gap-8 items-center md:w-2/3 rounded-lg sm:rounded-r-none overflow-hidden border-zinc-500/20 border-[0.7px] md:border-r-0 md:border-l-[0.7px] md:border-t-[0.7px] md:border-b-[0.7px]">
        ...
        <form className="w-9/10 flex mb-4 gap-2 md:gap-3 items-center">
            <input onChange={(e) => setInterimText(e.target.value)} type="text" value={interimText} placeholder="Reply..." className="rounded-3xl w-full border-[0.7px] outline-none border-zinc-500/50 focus-within:border-zinc-500/80 bg-zinc-900 transition-colors duration-200 px-4 py-2 md:py-3" />
            ...
            <button onClick={(e) => {
                ...
                setInterimText("");
            }} type="submit" > <ArrowUp className="h-7 w-7 bg-purple-600/60  rounded-sm text-zinc-200 cursor-pointer hover:bg-purple-500/60 p-1  transition-all duration-75" /></button>
            <button
                onClick={() => {
