...
export default function VolumeButton({ loud, setLoud, isTyping, voiceOn }: { loud: boolean, setLoud: any, isTyping: boolean, voiceOn: boolean }) {
    ...
    return <div className="w-full bg-zinc-800/30 flex justify-between items-center p-2 px-4 border-b border-white/5 shrink-0">
        ...
        >
            {loud ? <Volume2 className={`w-4  h-4 ${(isTyping || voiceOn) ? "text-red-500 animate-pulse" : "text-zinc-400"} md:w-5 md:h-5`} /> : <VolumeX className="w-4 h-4 md:w-5 md:h-5" />}
        </button>
    </div>
}