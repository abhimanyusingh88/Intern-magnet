...
export async function handleShortlist(email: string, id: string, setOpen: (open: boolean) => void, setSaving: (saving: boolean) => void, queryClient: any) {
    try {
        setSaving(true);
        const res = await fetch(`/api/shortlist/${id}/${email}`, {
            method: "PATCH",
            headers: {
